...
async def first_page_choose(message: Message, state: FSMContext):
    ...
    if user_data and user_data.get('refresh_token'):
        ...
        await state.set_state(SecondPage.workspace)

    else:
        await message.answer('Вас не найдено, надо регистрироваться', reply_markup=make_row_keyboard(reg_page))
        await state.set_state(FirstPage.reg_or_auth)





@router.message(FirstPage.reg_or_auth, F.text == 'Регистрация')
async def start_reg(message: Message, state: FSMContext):
    await message.answer(
        text="Даваи попробуем зарегаться, введи имейл",
        reply_markup=make_row_keyboard(prev_and_cancel)
    )
    await state.set_state(FirstPage.list_reg_state[0])

@router.message(FirstPage.reg_or_auth, F.text == 'Вход')
async def start_reg(message: Message, state: FSMContext):
    await message.answer(
        text="Введите имейл",
        reply_markup=make_row_keyboard(prev_and_cancel)
    )
    await state.set_state(FirstPage.list_log_state[0])
...
async def reg_prev(message: Message, state: FSMContext):
    ...
    if curr_state in SecondPage.list_workspace_state:
        index = SecondPage.list_workspace_state.index(curr_state)
        print(index)
        await message.answer(
            text="На главную",
        ...
        if index == 3 or 0:
            await state.set_state(SecondPage.list_workspace_state[0])
        else:
            await state.set_state(SecondPage.list_workspace_state[index-1])

    elif curr_state not in FirstPage.list_reg_state or curr_state == FirstPage.list_reg_state[0]:
        ...
        index = FirstPage.list_reg_state.index(curr_state)
        print(index)
        prev = await state.get_data()
        await message.answer(
            text=FirstPage.list_reg_text[index-1],
            reply_markup=make_row_keyboard(prev_and_cancel)
        )
        await state.set_state(FirstPage.list_reg_state[index-1])
...
@router.message(FirstPage.reg_or_auth)
...
@router.message(FirstPage.reg_email)
async def reg_email_choosen(message: Message, state: FSMContext):
    ...
    await state.set_state(FirstPage.reg_pass1)


@router.message(FirstPage.reg_pass1)
async def reg_pass1_choosen(message: Message, state: FSMContext):
    ...
    await state.set_state(FirstPage.reg_pass2)


@router.message(FirstPage.reg_pass2)
...
async def process_registration(message: Message, state: FSMContext, db=None):
    ...
    if registration_result is True:
        ...
        await state.set_state(FirstPage.reg_final)
...
@router.message(FirstPage.log_email)
async def log_email_choosen(message: Message, state: FSMContext):
    ...
    await state.set_state(FirstPage.log_pass)

@router.message(FirstPage.log_pass)
